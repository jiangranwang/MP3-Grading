...
with open(final_file, 'r') as f:
	lines = f.readlines()
	assert lines[0] == 'OK\n'
	assert int(lines[1].split()[2]) == 10 + (num_client - total_aborted) * 10, red[0] + 'inconsistent account 1 final balance' + red[1]
	assert int(lines[2].split()[2]) == 10 + (num_client - total_aborted) * 10, red[0] + 'inconsistent account 2 final balance' + red[1]
	assert lines[3] == 'COMMIT OK\n'

print(green[0] + 'isolation test 2 with ' + str(num_client) + ' clients check success' + green[1])