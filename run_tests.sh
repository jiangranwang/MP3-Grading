...
for server in ${servers[@]}; do
	./server $server config.txt > /dev/null &
	server_pids+=($!)
done
cd $curr_folder
...
tlimit () {
	...
	while true; do
		...
		for (( i=2; i<=11; i++ )); do 
			time ./client ${clients[$i]} config.txt < ../tests/isolation/limit/i$i.txt > /dev/null & 
		done
		sleep 2
...
itest3 () {
	...
	for (( i=0; i<$num_run; i++ )); do
		...
		for (( j=0; j<$abort; j++ )); do
			./client ${clients[$j]} config.txt < ../tests/isolation/test3/i$1-7-$i.txt > ../i3-$1-$i-$j.log &
			pids+=($!)
		done
		for (( j=$abort; j<$write+$abort; j++ )); do
			./client ${clients[$j]} config.txt < ../tests/isolation/test3/i$1-8-$i.txt > ../i3-$1-$i-$j.log &
			pids+=($!)
		done
		for (( j=$write+$abort; j<$read+$write+$abort; j++ )); do
			./client ${clients[$j]} config.txt < ../tests/isolation/test3/i$1-9-$i.txt > ../i3-$1-$i-$j.log &
			pids+=($!)
		done
		for pid in ${pids[@]}; do
			wait $pid
		...
		./client a config.txt < ../tests/isolation/test3/i$1-10-$i.txt > ../i3-$1-$i.log
		diff ../i3-$1-$i.log ../tests/isolation/test3/i$1-10-expected-$i.txt
		echo
	done

	read -p "Isolation 3 Test Number of Runs Passed (out of "$num_run")? " n
